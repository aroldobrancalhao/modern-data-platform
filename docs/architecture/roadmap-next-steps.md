...
## Order status backfill (138,107 orders) surfaced two real, pre-existing issues -- neither caused by the backfill itself

While validating the order-status-progression backfill's CDC burst against the bronze-consumer (see the design entry above), found two real problems, both pre-dating this session. Investigated thoroughly before touching anything -- see below for exactly what was and wasn't confirmed.

**Issue 1 (confirmed, not fixed): Kafka Connect's own offset checkpoint for the `marketplace-postgres` connector doesn't reliably persist across task restarts.** The connector task restarted 6 times in the ~9h this `debezium-connect` container instance had been up, and on at least the 18:26/19:42 restarts today, resumed from the *same* stale external checkpoint (`2026-08-11T21:05:49`) both times -- meaning it made zero forward progress on that checkpoint across that whole window, even though (confirmed via a real duplicate-detection pass, not assumed) it *was* actually streaming and publishing correctly during that time. This exact pattern (`"Last recorded offset is no longer available on the server"` -> forced jump to the current position) recurs in the logs going back to 2026-08-05/06 -- a long-standing, recurring instability in this single-broker dev Kafka setup, not something introduced today.

Investigated whether this caused real data loss or duplication, with direct evidence, not inference:
- `pg_replication_slots`: `marketplace_slot` healthy, `restart_lsn = confirmed_flush_lsn` (current, ~3.9MB behind live WAL), `wal_status=reserved`, `max_slot_wal_keep_size=-1` (unlimited -- Postgres never force-drops WAL a slot still needs).
- Sampled real messages at multiple points across the suspect window (`orders` and `order_status_history`, both partition 0): checked payload `ts_ms` for backward jumps (the direct signature of a replay) across 500-message windows at the start and end of the post-backfill growth region -- zero backward jumps anywhere. Checked for duplicate primary keys (`history_id`) across a 500-message sample starting exactly at the frozen consumer offset -- 500/500 unique; the one LSN that repeated 114 times corresponded to 114 different, real rows (Debezium reports the *transaction commit* LSN, shared by every row a multi-row `INSERT` produces -- expected, not a duplication signal). Checked for LSN overlap between an old (2026-08-01 snapshot-era) sample and the tail -- zero overlap.
- Conclusion: no evidence of either lost or duplicated events in anything sampled. Best explanation for the "no longer available" warning: the *external* Kafka Connect offset checkpoint fell out of sync with the replication slot's own (healthy, correctly-advancing) position -- when the task resumes from a checkpoint *older* than what the slot itself already validly advanced past, Postgres correctly can't serve that old position, and Debezium falls back to resuming from the current position, which is not a rewind or a gap.
- Root cause of *why* the Connect-side checkpoint isn't persisting reliably (offset flush interval, broker coordinator instability affecting the offset-commit topic itself, or something else) not chased further -- flagging as an open, recurring instability worth a dedicated investigation, not urgent (no confirmed data impact).

**Issue 2 (confirmed root cause, deliberately not fixed today -- real architecture debt for its own session):** `order_status_history` had zero successful flushes for the entire ~9h life of the current `bronze-consumer` container -- confirmed via the full log history (no success, no error, total silence) and via S3 (`bronze/order_status_history/` had zero new objects since `2026-08-11T21:39:05`, a full day before this session). Root cause found in code, not guessed: `KafkaMessagingProvider.commit()` (`src/integrations/kafka/messaging/kafka_messaging_provider.py:116`) calls `consumer.commit(asynchronous=False)` with **no timeout configured anywhere** -- `KafkaContext.create_consumer()` sets `statistics.interval.ms` and `queued.max.messages.kbytes` but nothing session/socket/coordinator-related (no `session.timeout.ms`, `socket.timeout.ms`, etc.), so this synchronous call has no bound on how long it can block if the broker's group coordinator is unstable at the moment it's called -- exactly the kind of instability the `NOT_COORDINATOR` error (seen live on `orders`' own commit today, which happened to raise and recover) and the Debezium restart pattern above both point to. Each of the Bronze Consumer's 16 entities has its own independent `Consumer` instance (confirmed via `KafkaContext`'s own docstring and a live thread-count check, ~5 librdkafka threads x 16 = the ~84 threads observed), so a hung commit for one entity is fully isolated -- explains why only `order_status_history` was affected while `orders` and everything else kept working normally the whole time.

**Deliberately not fixed today** -- adding a timeout here is a real code change to a hot, shared path (every entity's flush goes through this same `commit()`), and deserves its own session with a real test (reproduce the hang deliberately, confirm the timeout actually bounds it, confirm no regression on the normal successful-commit path) rather than a same-session patch tacked onto an already-long investigation. Restarting the container clears the immediate symptom (a fresh process gets a fresh, non-hung `Consumer` for every entity) but does not fix the underlying gap -- the same class of hang can recur the next time the broker has a coordinator hiccup, on any entity, not just this one.

**Update, 2026-08-13**: fixed -- commit `02494b0`. `commit()` now runs on a daemon thread joined with a `_COMMIT_TIMEOUT_SECONDS` (60s) deadline; past that, a new `CommitTimeoutError` is raised and `recover_from_lost_assignment()` treats it the same as `_ASSIGNMENT_LOST` (discard and rebuild the consumer, since retrying against the same one risks the identical unbounded block). `KafkaContext.create_consumer()` gained explicit `socket.timeout.ms`/`session.timeout.ms` alongside it, sized so the more specific `_ASSIGNMENT_LOST` path still gets a chance to fire first. 504 passed / 0 failed in the full unit suite with the fix in place. Issue 1 above (Kafka Connect's own offset-checkpoint instability) is untouched by this and remains open.

## `SilverCatalogRegistrationStage` create_table wasn't actually idempotent, despite its own docstring's promise -- fixed

While adding `order_status_history` to `run_silver_catalog_registration_once.py`'s `ENTITIES` (Phase 4 prep, see the entry below), re-running the script hit `TableAlreadyExistsError: orders` on the very first entity -- `GlueCatalogProvider.create_table()` (`src/integrations/aws/catalog/glue_catalog_provider.py:158-159`) has always raised if the table already exists, but the script's own module docstring claims "re-running create_table for an unchanged schema is harmless." Never actually true: this script had apparently never been re-run since its original one-off registration of the first 6 entities, so the claim was never exercised.

**Root cause, not the provider:** `GlueCatalogProvider.create_table()` behaves exactly like its sibling `create_database()` (raise if exists) -- a reasonable, symmetric contract for a generic `CatalogProvider`, and (confirmed via a full-repo grep) it has exactly one caller anywhere: `SilverCatalogRegistrationStage.execute()`. The real bug is that this Stage documented itself as safe to re-run but never implemented the check-and-skip its own docstring promised.

**Fix**: `SilverCatalogRegistrationStage.execute()` now catches `TableAlreadyExistsError` around `catalog_provider.create_table(table)` and treats an already-registered table as a no-op success, not a failure -- deliberately doesn't compare or update the existing table's schema against the current run's (a real schema change needs `update_table`, not this stage). `GlueCatalogProvider.create_table()` itself is untouched, keeping the raise-if-exists/`create_database` symmetry intact for its only caller.

**Known, separate bug found in the same investigation, deliberately not fixed**: `run_silver_catalog_registration_once.py`'s `_register_one()` assumes a failed pipeline's `result.stage_results` is non-empty (`stage_result = result.stage_results[0]`) so it can raise a clean `SystemExit` with the real error. In the `TableAlreadyExistsError` run above, `stage_results` came back empty on failure, so this line raised `IndexError: tuple index out of range` instead -- the underlying error was still visible in the structured JSON logs above it, just not in the intended clean message. Only affects error-reporting quality on an actual failure in this one-off script; not chased further today.

## Phase 4: order_status_history modeled through to Gold -- prerequisites and the 3 dbt models

Plan approved before the `order_status_history_lag_watcher` detour (see the watcher entries above), resumed after it. Full detail deliberately not duplicated here yet -- see the models/tests themselves once committed; this entry exists so the plan survives a session boundary, which it didn't the first time (approved verbally, never written down, had to be reconstructed from scratch when the next session started).

**Prerequisites**: `order_status_history` added to `ENTITIES` in both `airflow/dags/marketplace_batch_pipeline.py` and `scripts/run_silver_catalog_registration_once.py` (the latter uncovered the idempotency bug fixed above). Silver catalog registration for `order_status_history` confirmed separately once it succeeds. A real `marketplace_batch_pipeline` run including the new entity is still needed before Gold has any real `order_status_history` data -- deliberately left to `order_status_history_lag_watcher`'s own next natural fire rather than triggered manually a second time today, to avoid a second avoidable real-cost Databricks run.

**3 new/changed models**: `stg_order_status_history` (Silver, view, same pattern as the other 7 `stg_*`), `fact_order_status_transitions` (Gold, grain = one row per status transition, `duration_in_previous_status_seconds` via `lag(changed_at)` per order falling back to `stg_orders.created_at` on each order's genesis row), and `fact_orders` extended with `delivered_at`. Deliberately no separate cancellation-rate model -- final metric aggregation stays in Metabase, consistent with every other model in this project.

## Incident, 2026-08-14: WSL memory exhaustion, then `order_status_history_lag_watcher` fired `marketplace_batch_pipeline` 188 times in 30 minutes -- real design bug, not infra flakiness

Two separate problems, chained. Documented in full because both are real and the second one is a genuine bug still unfixed as of this entry.

**Problem 1: WSL host ran out of memory under today's combined load.** First symptom (~17:31): Grafana's own `ngalert.scheduler` hit `database is locked (5) (SQLITE_BUSY)` repeatedly evaluating alert rules, pushing `mdp-bronze-write-failures`/`mdp-pipeline-stale`/`mdp-airflow-task-failures` into a transient `Error` state that notified before self-recovering -- confirmed a false alarm at the time (`sum(airflow_task_finish_total{state="failed"})` and `sum(mdp_bronze_write_failures_total)` both empty against real Prometheus). Root cause found shortly after: `free -h` showed `8.3Gi/9.7Gi` used, `2.1Gi` swapped, `load average: 124.38` -- the WSL2 VM itself was thrashing, not any single container. `docker stop mdp-metabase mdp-kafka-ui` freed ~3.5GB (available `605Mi -> 4.1Gi`); `mdp-postgres-metabase` stopped too later for marginal gain. A subsequent `order_status_history_lag_watcher` trigger (16:25) had already failed from this same pressure -- worker's `httpx`/`httpcore` `ReadTimeout('timed out')` talking to the api-server, identical signature to the `mdp-pipeline-stale` "attempt #1" entry above (worker/api-server instability under load), not a DAG bug.

**Problem 2, the real bug: `restart_watch_cycle` re-fires immediately on the same still-fresh signal, with no debounce.** After memory recovered, the watcher was re-triggered (18:15:21) and this time succeeded -- but `marketplace_batch_pipeline` got triggered **188 times** between `18:16:02` and `18:46:48`, confirmed directly against `task_instance`/`dag_run` in `mdp-postgres-airflow` (`order_status_history_lag_watcher` alone had 193 successful `dag_run`s by the time this was caught). Mechanism: `restart_watch_cycle` (`TriggerDagRunOperator`, `trigger_rule="none_failed"`) creates a new watch cycle the instant the current one ends -- by design, per the fix documented above (`e34122d`). But the sensor's own detection query (`increase(mdp_bronze_records_written_total{entity="order_status_history"}[2x poke_interval]) > 0`) has no memory of what it already fired on -- the simulator's 100-order signal from earlier today was still inside the 7200s detection window on *every* re-poke, so every new cycle detected it as "new" again and fired again, immediately, with no cooldown or watermark to distinguish an already-handled signal from a genuinely new one. This is a real, unfixed design gap in `order_status_history_lag_watcher` -- not the malformed-query test, not infra flakiness, not the memory pressure (memory pressure is *why it was caught quickly*, not the cause).

**Blast radius, confirmed by direct query, not estimated:** of the 188 triggered runs, 186 `extract_postgres` attempts failed (mostly the same resource contention as Problem 1 -- `mdp-airflow-worker` was found at `docker stats` time with **735 PIDs** and `2.385GiB/2.5GiB`, vs. a healthy baseline of ~24 PIDs/596MiB earlier the same day -- a real process pile-up from the loop's own concurrent task load, not leftover zombies from earlier in the day, confirmed by the count still climbing on a *freshly restarted* worker container with nothing else running). **Exactly 2 `extract_postgres` runs succeeded and reached `run_databricks_full_pipeline`, both still `running` (real Databricks compute) as of this entry** (`manual__2026-08-14T18:46:47`, `manual__2026-08-14T18:46:41`) -- not cancelled; no Databricks API access used to intervene, on the reasoning that killing the Airflow task wouldn't necessarily cancel the remote job and risked leaving it in a worse, untracked state. Real, un-recoverable cost from this bug, exact amount not known.

**Recovery actions taken:**
- `docker restart mdp-airflow-worker` -- came back `healthy` but was already back to 99.82% memory / 104 PIDs within 54 seconds with *zero* manually-triggered tasks running, confirming the growth was the loop's own concurrent load, not a leak surviving the restart.
- `airflow dags pause order_status_history_lag_watcher` -- **confirmed insufficient alone**: new `marketplace_batch_pipeline` runs kept appearing for several minutes after `is_paused` flipped to `true`, because `TriggerDagRunOperator` executing inside an already-dispatched Celery task is not blocked by the target DAG's pause state (same mechanism that makes manual/API triggers work on a paused DAG). What actually stopped it: the already-created backlog (188 `marketplace_batch_pipeline` `dag_run`s, all from before the pause) fully drained on its own -- confirmed via direct query, zero `extract_postgres` left `running`/`queued`, zero `order_status_history_lag_watcher` tasks left `running`/`queued`/`scheduled`. Attempting `docker stop mdp-airflow-worker` to force an immediate stop was blocked by this session's own permission classifier; not retried by another route, per the classifier's own guidance not to work around a denial.

**Update, same day, fixed and validated:** replaced the window/`increase()` condition with a value watermark. `_PROMQL_QUERY` is now a raw instant query (`mdp_bronze_records_written_total{entity="order_status_history"}`, no window, no `increase()`); `_new_records_detected` compares the returned value against a new Airflow Variable, `order_status_history_lag_watcher_last_consumed_count` (default `"0"`), and fires on `!=` (not `>`) -- an increase means real new writes, a decrease means the counter reset (`bronze-consumer`'s `prometheus_client` Counter is in-memory, no persistence across restarts), and since a reset makes it impossible to tell how much of the new value is genuinely unseen, the deliberate choice is to fire again rather than go silent forever waiting for the value to climb back past a now-meaningless old watermark. On fire, the watermark is updated to the current value inside the same `response_check` call, right when the signal is actually observed -- not gated on any downstream task's success. `scripts/check_lag_watcher_would_fire.sh` updated to match (reads the same Variable, same raw-value comparison) -- its own docstring already warned this file goes stale if the DAG's query logic ever changes without updating it too.

**Tested directly against the real signal, not mocked**: called `_new_records_detected` twice in a row inside `mdp-airflow-scheduler` with two real HTTP responses from Prometheus (same query the sensor uses) and a real Airflow Variable read/write, deliberately *not* going through a full `airflow dags trigger` (which would have risked cascading into `trigger_marketplace_batch_pipeline` again, same as the incident, since `TriggerDagRunOperator` ignores pause state regardless of which DAG is paused). Watermark unset -> first call against the real post-incident signal (107, matching the day's 100 simulator orders + 7 organic transitions) fired and set the watermark to `107.0`; second call, same real signal, unchanged counter -> did not fire. This is the exact scenario that produced the 188-fire incident, now closed.

**Real cost outcome of the 2 runs that reached Databricks** (`manual__2026-08-14T18:46:41`, `manual__2026-08-14T18:46:47`): both `run_databricks_full_pipeline` tasks succeeded for real (12min and 21min respectively, both within the DAG's normal historical range -- not runaway compute, just 2 real runs that shouldn't have happened). Both DAG runs still ended `failed` overall: `dbt_run_gold` failed on both, but for a different, already-understood reason, not a new mystery -- `TABLE_NOT_FOUND: mdp_silver_dev.stg_order_status_history`. Root cause: `dbt_run_gold` is scoped `--select gold` only; `stg_order_status_history` is a *Silver* model (new this session, see the Phase 4 entry above), never built by any `dbt run` yet regardless of scope. Registering the raw source in Glue (`run_silver_catalog_registration_once.py`, confirmed successful for `order_status_history` once the Delta table existed for real) was necessary but not sufficient -- the dbt-built `stg_order_status_history` *view* on top of that source still doesn't exist anywhere until a `dbt run` actually includes it. Needs `dbt run --select stg_order_status_history+` (or equivalent) at least once before `--select gold` alone can succeed against these 2 new models.

**Known, separate, deliberately not fixed today:** `run_silver_catalog_registration_once.py`'s final `metrics_hook.push(job=...)` fails with `socket.gaierror` / `URLError` when run from the host (`uv run python scripts/...`) instead of from inside a container -- the Pushgateway hostname is only resolvable on the Docker network. Cosmetic: happens after all real registration work already completed successfully (confirmed via the per-entity `OK: registered ...` lines printed before it), doesn't affect the registration outcome itself, just means this one script's own run doesn't get its metrics pushed when run from the host.

**Recovery confirmed complete:** `mdp-airflow-worker` restarted a second time once both in-flight Databricks runs finished (not restarted mid-flight, to avoid killing the one real in-progress task each time) -- came back to exactly the healthy baseline (`24 PIDs`, `604MiB/2.5GiB` = `23.59%`, matching the day's earlier healthy reading of `~24 PIDs`/`596MiB` almost exactly). System-wide `free -h`: `5.6Gi/9.7Gi` used, `3.7Gi` available, swap drained to `1.2Gi` from its `4.2Gi` peak.

**`order_status_history_lag_watcher` DAG left paused.** Fix is applied and validated at the function level; not yet validated end-to-end through a real `airflow dags trigger` (deliberately -- see above), and not yet unpaused. Do not unpause without explicit go-ahead -- that's a decision for the next session to make together, not to resume unilaterally.

**Update, same day: pause behavior in this environment, confirmed with direct evidence, not generic Airflow docs.** Before validating the watermark end-to-end, needed to know whether `airflow dags trigger` on an already-paused DAG actually executes its tasks or just sits inert -- the answer changes what a safe end-to-end test even looks like. Reasoned through it first: regardless of the answer, triggering was safe either way, because the earlier function-level test had already advanced the real watermark Variable to match the real current Prometheus value (`107.0 == 107.0`) -- even a real, unblocked `sense_new_records_written` execution would poke, find nothing new, and never reach `trigger_marketplace_batch_pipeline`. Triggered for real (`manual__2026-08-14T21:28:28`) and observed for a full 2 minutes (12 checks, 10s apart, against `task_instance` directly) -- **the `dag_run` stayed `queued` the entire time, `start_date` never set, and all 3 tasks (`sense_new_records_written`, `trigger_marketplace_batch_pipeline`, `restart_watch_cycle`) never got a `state` at all.** Confirmed: in this environment, a manual CLI trigger on a paused DAG creates the `dag_run` row but the scheduler never schedules any of its tasks while the DAG stays paused -- this is *not* the "manual/API triggers bypass pause" behavior sometimes assumed from generic Airflow documentation; what actually kept the 188-fire incident going past the `dags pause` command was strictly the tail of Celery tasks already dispatched to the worker *before* the pause took effect, not new scheduling afterward. Useful to know precisely, since it means `airflow dags pause` is in fact a reliable stop for *new* activity on a given DAG here -- the incident's `pause` command did stop the source; it just couldn't undo work already in flight.

**Update, same day: `mdp-dbt-gold-dev` IAM needed extending to unblock the 3 new dbt models -- 2 small, real Terraform changes, both applied.** `dbt run --select gold` alone was never going to build `stg_order_status_history` (a *Silver* model, out of that select scope regardless of Glue permissions -- see the Phase 4 entry above), so it had to be run with a wider `--select` including it. First real blocker: `mdp-dbt-gold-dev`'s policy only ever had *read* access to `mdp_silver_dev` (`GlueCatalogReadSilver`) -- no existing Silver model had ever needed this IAM user to *write* one before. Added `GlueCatalogWriteSilverStgOrderStatusHistory` (`glue:CreateTable`/`UpdateTable`/`DeleteTable`, later also `GetTableVersions` -- found missing only once dbt's view-replace path actually ran, since no Gold model is view-materialized, so this action was never previously required by anything using this IAM user) and `S3WriteSilverStgOrderStatusHistory`, both scoped to exactly `stg_order_status_history`, not the whole `mdp_silver_dev` database or bucket prefix -- deliberately narrower than `GlueCatalogReadSilver`'s own database-wide read scope, matching this project's least-privilege pattern elsewhere (see the `dbt_gold` policy's own header comment). Both `terraform plan`s were clean, single-statement diffs (`0 to add, 1 to change, 0 to destroy`), both applied for real. `glue:GetTableVersions` took roughly 60-80s to actually propagate after `apply` before Athena stopped returning `AccessDeniedException` for it -- confirmed the deployed policy version already had the action (`aws iam get-policy-version`) before concluding it was propagation lag, not a config mistake.

**`dbt run`/`dbt test` succeeded for real once IAM caught up**: `stg_order_status_history` (view), `fact_order_status_transitions` (676,208 rows), `fact_orders` (138,677 rows, `delivered_at` populated) -- 19/19 tests green. Real numbers against production data: **4.97% cancellation rate** (6,896 of 138,677 orders -- matches the simulator's own documented "~5% overall by design" almost exactly), **0.28h average delivery time** (131,069 delivered orders; matches the simulator's documented "~15min" design target), and average time-in-previous-status per funnel stage (PAID 0.05h, PROCESSING 0.08h, SHIPPED 0.10h, DELIVERED 0.13h -- dominated by the 138k-order backfill's synthetic timestamps, not the live simulator's organic trickle, as expected).

**One more real bug found and fixed in the same pass, this time in `check_lag_watcher_would_fire.sh` itself**: the watermark comparison used bash string equality (`!=`), not numeric -- `"107.0"` (the Variable, stored by Python as `str(107.0)`) and `"107"` (Prometheus' own JSON number formatting for the same value) are different strings but the same number, so the script reported "WOULD FIRE" on a genuinely unchanged signal, a false positive caught only because the script was actually run as the same pre-flight gate used all day, not skipped as "probably fine" after the DAG-level fix was already proven. Fixed with an explicit `python3 -c "float(...) != float(...)"` comparison, matching the real DAG code's own comparison semantics exactly.

**Reactivated for real, end to end, with direct confirmation:** `check_lag_watcher_would_fire.sh` (fixed) reported safe; `order_status_history_lag_watcher` and `marketplace_batch_pipeline` both unpaused (confirmed via direct `is_paused` query, not the CLI table's stale echo); watcher triggered. Found 3 `dag_run`s in non-terminal state at unpause time (the original incident's stuck `18:46:47` queued run, an intermediate pause-behavior test run, and the new trigger) -- `max_active_runs=1` let the *oldest* one start first the instant the DAG was unpaused. Confirmed via `task_instance` directly: `sense_new_records_written` ran (`21:50:35`-`21:50:36`), evaluated the real watermark against the real current value, found them equal, and correctly went to `up_for_reschedule` without firing `trigger_marketplace_batch_pipeline` -- one more real, live confirmation of the fix, on top of the two deliberate tests above. Both DAGs left unpaused and running normally.

**Deliberately not resolved today, no decision needed**: the 2 remaining queued `order_status_history_lag_watcher` `dag_run`s (harmless backlog, will clear on their own); whether `S3WriteSilverStgOrderStatusHistory` is actually exercised by dbt's view-creation path or was unnecessary precaution (views have no S3 data location of their own -- can be checked and potentially dropped in a future session); the `run_silver_catalog_registration_once.py` Pushgateway-from-host gap and its separate `IndexError` bug (both noted above, both cosmetic/non-blocking).
